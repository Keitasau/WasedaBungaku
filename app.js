...
const STORAGE_KEY = 'wbvm_bungaku_v1';
const TOTAL = 129;
const SMALL_STEP_SIZE = 10;

// Topic → creature/environment config (Waseda Bungaku edition)
const TOPIC_META = {
  'Language Lagoon': {
    emoji: '🗣️', color: '#29b6f6',
    creatures: ['🐬','💬','🐠','🌊','📖'],
    corals: ['🪸','🌿','🐚','🌊'],
    bgCreature: '🐬',
    label: '言語・コミュニケーション'
...
const SHARK_HOME = [
  ...
  "130 words await! 🌊", "The ocean of knowledge awaits!",
  "Swim through the words today!", "Let's explore the deep!",
];

// Correct burst emojis by topic (Waseda Bungaku edition)
const BURST_CORRECT = {
  'Language Lagoon': ['💬','✨','📖','⚡'],
  'History Reef':    ['📜','✨','⚓','🔥'],
  'Culture Cove':    ['🎨','✨','🌸','💜'],
...
const App = {
  ...
  getTodayDay() {
    try {
      let firstDate = localStorage.getItem('wbml_start');
      if (!firstDate) {
        firstDate = new Date().toDateString();
        localStorage.setItem('wbml_start', firstDate);
      }
      const diff = Math.floor((new Date() - new Date(firstDate)) / 86400000);
      return Math.min(17, Math.max(1, diff + 1));
    } catch(e) { return 1; }
  },
